...
//           Log.d("test", String.valueOf(MainActivity.is32));
        if (EspFreeMainActivity.gameType == 1 && EspFreeMainActivity.is32) {
            Start(ctx,1,1);
...
//            Start(ctx,1,2);
//        }
        else if (EspFreeMainActivity.gameType == 2 && EspFreeMainActivity.is32) {
            Start(ctx,2,1);
        }
//        else if (MainActivity.gameType == 2 && MainActivity.is64) {
//            Start(ctx,2,2);
//        }
        else if (EspFreeMainActivity.gameType == 3 && EspFreeMainActivity.is32) {
            Start(ctx,3,1);
        }
//        else if (MainActivity.gameType == 3 && MainActivity.is64) {
//            Start(ctx,3,2);
//        }
        else if (EspFreeMainActivity.gameType == 4 && EspFreeMainActivity.is32) {
            Start(ctx,4,1);
        }
            ...
            if (!isRunning) {
                ...
                if (Init() < 0) {
                    new Handler(Looper.getMainLooper()).post(() -> {
                        Toast.makeText(FreeOverlay.this, "Game Not Running!", Toast.LENGTH_SHORT).show();
                        FreeOverlay.this.stopSelf();
                    });
                } else {
                    isRunning = true;
                    startService(new Intent(this, FreeService.class));
                    SettingValue(-1,true);
                    SettingValue(85,true);
...
//    }
    ...
    public static native void SettingValue(int i,boolean j);
